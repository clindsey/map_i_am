    ...
    MapIAm.prototype.world_map_scene = function() {
      ...
      for (_i = 0, _len = countries.length; _i < _len; _i++) {
        ...
        this.build_country(country, 1, 0, 0, this.stage_half_width, this.stage_half_height, __bind(function(region_shape) {
          return (__bind(function(country_index, country_name) {
            return region_shape.onClick = __bind(function() {
              this.stage.removeAllChildren();
              if (country_name === 'Europe') {
                return this.europe_map_scene(country_index);
              } else {
                return this.country_map_scene(country_index);
              }
            }, this);
          }, this))(country_index, country.name);
        }, this));
      }
      return this.stage.update();
    };
    MapIAm.prototype.europe_map_scene = function(country_index) {
      var background, bounds, centering_x, centering_y, country, country_name, european_country_index, offset, scale, x_scale, xy_delta, y_scale, _i, _len;
      background = this.create_background();
      this.stage.addChild(background);
      background.onClick = __bind(function() {
        this.stage.removeAllChildren();
        return this.world_map_scene();
      }, this);
      this.country_name_dom.innerHTML = 'Europe';
      ...
      for (_i = 0, _len = european_countries.length; _i < _len; _i++) {
        country = european_countries[_i];
        european_country_index += 1;
        country_name = country.name;
        country_index += 1;
        this.build_country(country, scale, offset.x, offset.y, centering_x, centering_y, __bind(function(region_shape) {
          return (__bind(function(european_country_index, country_name) {
            return region_shape.onClick = __bind(function() {
              this.stage.removeAllChildren();
              return this.european_country_map_scene(european_country_index);
            }, this);
          }, this))(european_country_index, country.name);
        }, this));
      }
      return this.stage.update();
    };
    MapIAm.prototype.european_country_map_scene = function(european_country_index) {
      var background, bounds, centering_x, centering_y, country, offset, scale, x_scale, xy_delta, y_scale;
      background = this.create_background();
      this.stage.addChild(background);
      background.onClick = __bind(function() {
        this.stage.removeAllChildren();
        return this.europe_map_scene(143);
      }, this);
